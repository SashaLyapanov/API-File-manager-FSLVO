...
public class CompetitionService {
    ...
    @Value("${competitions.dir}")
    private String uploadDirectory;


    public ResponseEntity<String> uploadFiles(String competitionId, MultipartFile[] files) {
        try {
            // Очищаем директорию
            cleanDirectory(uploadDirectory + competitionId + "/");
            ...
            if (files != null && files.length > 0) {
                ...
                for (MultipartFile file : files) {
                    if (file != null && !file.isEmpty()) {
                        ...
                        Path filePath = Paths.get(uploadDirectory + competitionId + "/", fileName);
    ...
    public byte[] downloadFile(String id, String fileName) throws IOException {
        String filePath = uploadDirectory + id + "/" + fileName;
